...
DateTime& DateTime::operator=(const std::string& value)
{
	if (value.size() != 15 || value[8] != 'T' ||
		!std::isdigit(value[0]) || !std::isdigit(value[1]) || !std::isdigit(value[2]) || !std::isdigit(value[3]) ||
		!std::isdigit(value[4]) || !std::isdigit(value[5]) ||
		!std::isdigit(value[6]) || !std::isdigit(value[7]) ||
		!std::isdigit(value[9]) || !std::isdigit(value[10]) ||
		!std::isdigit(value[11]) || !std::isdigit(value[12]) ||
		!std::isdigit(value[13]) || !std::isdigit(value[14]))
	{
		return *this;
	}
...
DateTime& DateTime::operator=(const char* value)
{
...
{
	return CompareDates(a);
}

bool DateTime::operator>(const DateTime& a) const
{
	return a.CompareDates(*this);
}
